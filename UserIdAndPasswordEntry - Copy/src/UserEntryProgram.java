...
public class UserEntryProgram {
    ...
    public static String inputId() {
        ...
        while (!valid) {
            ...
            if (valid = validUserIdLength(iUserId)) {

                if (valid = validUserId(iUserId)) {  // [0-9]{3,12} used to keep numbers from exceeding 2
    ...
    public static Boolean validUserId(String iUserId){
        return iUserId.matches("[a-z|A-Z]{6,10}[0-9]{2}") && !iUserId.matches(".*\\W.*");
    }

    public static Boolean validUserIdLength(String iUserId){
        return iUserId.trim().length() >= 8 && iUserId.trim().length() <= 12;
    }
    ...
    public static String inputPassword(){
        ...
        while(!valid){
            ...
            if (valid = validUserPasswordLength(iUserPassword)) {
                if (valid = validUserPassword(iUserPassword)) {
    ...
    public static Boolean validUserPassword(String iUserPassword){
        return iUserPassword.matches(".*[a-z]{1,10}.*") && iUserPassword.matches(".*[0-9]{1,10}.*") && iUserPassword.matches(".*[A-Z]{1,10}.*") && !iUserPassword.matches(".*\\W.*");
    }

    public static Boolean validUserPasswordLength(String iUserId){
        return iUserId.trim().length() >= 6 && iUserId.trim().length() <= 12;
    }
